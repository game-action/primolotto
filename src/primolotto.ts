...
export class Primolotto {
  ...
  public async summary(): Promise<Summary> {
    const response = await this.axiosInstance.get(this.uri.summary);
    const $ = cheerio.load(response.data);
    return new Summary(parseInt($('.raffle-balance').text()));
  }
  ...
  private async fetchGridToken(): Promise<GridInfo> {
    const response = await this.axiosInstance.get(this.uri.gridUri);
    const $ = cheerio.load(response.data);
    return {
  ...
  private async postGrid(grid: GridInfo): Promise<boolean> {
    ...
    const response = await this.axiosInstance.post(this.uri.playUri, postData, axiosConfig);
    return response.status === 200;
  }

  private async tokenConnect(): Promise<string> {
    const response = await this.axiosInstance.get(this.uri.connect);
    const $ = cheerio.load(response.data);
